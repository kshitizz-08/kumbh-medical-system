...
type ProfileProps = {
  devotee: Devotee & { medical_records: MedicalRecord | null };
  refreshToken?: number;
  onClose: () => void;
  onRecordIncident: (devoteeId: string, devoteeName: string) => void;
};

export default function MedicalProfile({ devotee, refreshToken, onClose, onRecordIncident }: ProfileProps) {
  const { t } = useI18n();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{devotee.full_name}</h2>
