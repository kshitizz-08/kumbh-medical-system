...
type SelfieCaptureProps = {
  ...
  onCapture: (imageData: string, faceDescriptor: number[] | null) => void;
  onClose: () => void;
...
export default function SelfieCapture({ onCapture, onClose }: SelfieCaptureProps) {
  ...
  const capturePhoto = async () => {
    ...
    // Compute face descriptor for this captured frame
    ...
    try {
      ...
      if (detectionWithDescriptor?.descriptor) {
        ...
        console.warn('No face descriptor could be computed from captured image.');
      }
    } catch (err) {
      console.error('Failed to compute face descriptor from captured image:', err);
    }
  ...
  const confirmCapture = async () => {
    if (capturedImage) {
      ...
      onCapture(capturedImage, faceDescriptor);
      onClose();
    }
  };
